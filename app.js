...
// ✅ 全局标记（跨 session）
const LS_MARKED_SET = "marked_qids_v3";
...
// ✅ 题内画笔模式开关（之前缺失会导致 setDrawMode is not defined）
function setDrawMode(on){
  ...
  if(btnDrawMode){
    btnDrawMode.textContent = drawMode ? "🖍️ 题内画笔：开" : "🖍️ 题内画笔：关";
    btnDrawMode.classList.toggle("marked", drawMode);
  }
}
 ...
 * 未做题优先抽题（已做题尽量 ≤ maxSeenRatio），未做不足时自动用已做补齐。
 * 目标：避免“未做题很少时”因为筛空导致崩溃；全程静默不提示用户。
 * 说明：这里的“已做”来自 startSession() 里统计出来的 seen Set。
 */
function pickMostlyUnseen(list, count, seenSet, maxSeenRatio = 0.01){
  const B = Array.isArray(list) ? list : [];
  const want = Math.min(Math.max(0, Number(count) || 0), B.length);
  if(!want) return [];

  const S = (seenSet && typeof seenSet.has === "function") ? seenSet : new Set();

  const unseen = [];
  const seen = [];
  for(const q of B){
    const id = (q && q.id != null) ? String(q.id) : "";
    if(id && S.has(id)) seen.push(q);
    else unseen.push(q);
  }

  // Fisher–Yates 洗牌（比 sort(random) 更稳）
  const fyShuffle = (arr) => {
  ...
  const maxSeen = Math.max(1, Math.floor(want * maxSeenRatio));
  const picked = [];

  // ① 优先抽未做
  picked.push(...unseen.slice(0, want));

  // ② 未做不足则补已做（尽量 ≤1%）；若仍不足继续补齐，保证不崩
  if(picked.length < want){
    const need = want - picked.length;
    const allow = Math.min(need, maxSeen);
    picked.push(...seen.slice(0, allow));

    if(picked.length < want){
      picked.push(...seen.slice(allow, allow + (want - picked.length)));
    }
  }

  // ③ 极端兜底：仍不足则从全量剩余补齐
  if(picked.length < want){
    const used = new Set(picked.map(x => (x && x.id != null) ? String(x.id) : ""));
    const rest = B.filter(q => {
      const id = (q && q.id != null) ? String(q.id) : "";
      return !used.has(id);
    });
    fyShuffle(rest);
...
// ✅ 全局标记：加载/保存
function loadMarkedGlobal(){
  const arr = loadLS(LS_MARKED_SET, []);
  if(Array.isArray(arr)){
    markedGlobal = new Set(arr.map(x => String(x)));
  }else{
    markedGlobal = new Set();
  }
}
function saveMarkedGlobal(){
  saveLS(LS_MARKED_SET, Array.from(markedGlobal));
}
loadMarkedGlobal();
...
function refreshSetupStats(){
  ...
  if(statsBox){
    statsBox.innerHTML = `
      ...
      <div>全局标记：<b>${markedGlobal.size}</b> 题</div>
    `;
  }
...
safeOn(btnResetLocal, "click", () => {
  if(!confirm("确定清空本地数据吗？（题库/分类/笔记/全部记录/全局标记）")) return;
  ...
  localStorage.removeItem(LS_MARKED_SET);
...
// ======================= ✅ 全局标记功能 =======================
function isMarkedGlobal(qid){
  return markedGlobal.has(String(qid));
}
function renderMarkState(){
  if(!btnMark || !playlist.length) return;
  const qid = playlist[idx];
  const on = isMarkedGlobal(qid);
  btnMark.textContent = on ? "⭐ 标记：开" : "⭐ 标记：关";
  btnMark.classList.toggle("marked", on);
}

// ✅ 切换全局标记，并同步写入“当前 session item”（导出更直观）
function toggleMark(){
  if(!playlist.length) return;
  const qid = String(playlist[idx]);

  const nowOn = !markedGlobal.has(qid);
  if(nowOn) markedGlobal.add(qid);
  else markedGlobal.delete(qid);
  saveMarkedGlobal();

  // 同步到当前 session item（可选但实用）
  if(session){
...
function buildSummaryCircles(){
  ...
  for(let i=0;i<total;i++){
    ...
    // ✅ 圈圈右上角⭐：按全局标记判断
    if(isMarkedGlobal(qid)) b.classList.add("marked");
...
safeOn(btnFinish, "click", () => {
  ...
  session.items.forEach(it => {
    ...
    // ✅ 提交时再同步一次全局标记
    it.marked = isMarkedGlobal(it.qid);
  });
...
safeOn(btnStart, "click", () => {
  ...
  // ✅ 统计历史做题：seen / wrong（标记用全局 markedGlobal）
  ...
  allSessions.forEach(s => {
    (s.items || []).forEach(it => {
      ...
      seen.add(qid);
      if(it.isCorrect === false) wrong.add(qid);
    });
  });

  // ✅ 错题池 = wrong ∪ markedGlobal
  const reviewSet = new Set([...wrong, ...markedGlobal]);
  ...
  if(poolMode === "wrong"){
    pool = pool.filter(q => reviewSet.has(String(q.id)));
  ...
  let picked;
if(poolMode === "all"){
  // 未做题优先，已做题尽量 ≤1%，未做不足时自动补齐（静默）
  picked = pickMostlyUnseen(pool, count, seen, 0.01);
}else{
